...
def fe(df, df_2):
    ...
    df['NAME_FAMILY_STATUS_IS_MARRIED'] = df['NAME_FAMILY_STATUS'].map(
        {'Civilmarriage': 1, 'Married': 1, 'Separated': 0, 'Single/notmarried': 0, 'Unknown': 0, 'Widow': 0})
    df['NAME_INCOME_TYPE_IS_WORKING'] = df['NAME_INCOME_TYPE'].map(
        {'Businessman': 0, 'Commercialassociate': 0, 'Maternityleave': 0, 'Pensioner': 0, 'Stateservant': 1,
         'Student': 0, 'Unemployed': 0, 'Working': 1})
    df['NAME_EDUCATION_TYPE_IS_WELL_EDUCATED'] = df['NAME_EDUCATION_TYPE'].map(
        {'Academicdegree': 1, 'Highereducation': 1, 'Incompletehigher': 1, 'Lowersecondary': 0,
         'Secondary/secondaryspecial': 0})
...
def load_create_intermidiate(subset=0):
    # load data
...
def scale_transform():
    ...
    df_application_test = apply_transform(df_application_test,
                                          ["scale", "log_transform", "min_max_transform", "standard_scale"])
    save_intermediate(df_application, df_application_test)


def handle_categoricals():
    df_application, df_application_test = load_intermediate()
    print(psutil.Process(pid=os.getpid()).memory_full_info())
...
def prep_6():
    ...
    get_agg_numerics_data(df_application, 'installments_payments', df_installments_payments, ['SK_ID_CURR'], output,
                          True)
    get_agg_numerics_data(df_application_test, 'installments_payments', df_installments_payments, ['SK_ID_CURR'],
                          output, False)
    ...
    get_agg_numerics_data(df_application_test, 'previous_application', df_prev_application, ['SK_ID_CURR'], output,
                          False)
...
def align_train_test_finalize():
    ...
    df_application = remove_categoricals(df_application)
    df_application_test = remove_categoricals(df_application_test)
...
def log_result(result):
    ...
    a = result[0]
    a_test = result[1]
    print('logging multiprocess result: {} {}'.format(a.shape if a is not None else 0,
                                                      a_test.shape if a_test is not None else 0))
    b.append(a)
...
def runInParallel(fns, final=False):
    ...
    results = []
    for i in args:
        results.append(pool.apply_async(agg_single, [i], callback=log_result, error_callback=log_error))
...
def get_agg_numerics_data(df, file_name, additional_data, exclude_cols, output, train=True):
    ...
    for groupby_cols, specs in AGGREGATION_RECIPIES:
        ...
        for select, agg in specs:
            counter += 1
            groupby_aggregate_name = '{}_{}_{}_{}_{}'.format(str(counter), file_name, '_'.join(groupby_cols), agg,
                                                             select)
...
def dummy(msg):
    print("skipping {}".format(msg))

def main(pipeline):
        load_intermediate() if pipeline["load_intermediate"] else dummy("load_intermediate")
        clean() if pipeline["clean"] else dummy("clean")
        scale_transform() if pipeline["scale_transform"] else dummy("scale_transform")
        feature_engineer() if pipeline["feature_engineer"] else dummy("feature_engineer")
        handle_categoricals() if pipeline["handle_categoricals"] else dummy("handle_categoricals")
        prep_6() if pipeline["prep6"] else dummy("prep6")
        align_datasets() if pipeline["align_datasets"] else dummy("align_datasets")
